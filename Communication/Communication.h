...
public:
  ...
  void Broadcast (int root, vector<complex<double> > &buff);
...
// 		    int recvProc,       blitz::Array<int,1> &recvBuff);
  ...
  complex<double> Sum (complex<double> a);
  void Sum (vector<complex<double> > &sendBuff, vector<complex<double> > &recvBuff);
  ...
  void Sum(Eigen::MatrixXd &sendBuff, Eigen::MatrixXd &recvBuff);
