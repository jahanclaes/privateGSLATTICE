...
void 
CommunicatorClass::Broadcast (int root, float &val)
{  MPI_Bcast(&val, 1, MPI_FLOAT, root, MPIComm); }
...
void 
CommunicatorClass::Broadcast (int root, vector<complex<double> > &buff)
{
  MPI_Bcast(&(buff[0]), 2*buff.size(), MPI_DOUBLE, root, MPIComm);
}
...
void 
CommunicatorClass::Sum(Eigen::MatrixXd &sendBuff, Eigen::MatrixXd &recvBuff)
{
  double *sendPtr=(double*)(sendBuff.data());
  double *recvPtr=(double*)(recvBuff.data());
  int count=sendBuff.cols()*sendBuff.rows();
  MPI_Reduce(sendPtr, recvPtr, count, MPI_DOUBLE, MPI_SUM, 0, 
	     MPIComm);
...
///resulting sum.
void 
CommunicatorClass::Sum (vector<complex<double> > &sendBuff, vector<complex<double> > &recvBuff)
{
  complex<double> *sendPtr = &(sendBuff[0]);
  complex<double> *recvPtr = &(recvBuff[0]);
  int count = sendBuff.size();
...
///other processors return 0;
complex<double>
CommunicatorClass::Sum (complex<double> a)
{
  complex<double> sum;
  MPI_Reduce(&a, &sum, 2, MPI_DOUBLE, MPI_SUM, 0, MPIComm);
  if (MyProc()==0)
    return sum;
  else
    return (0.0);
