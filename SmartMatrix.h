...
class SmartMatrix
{
 public:
  ...
  void SaveCol(int i)
  {
    savedCol(Range::all())=M(i,Range::all());
  }
  void SaveRow(int i)
  {
    savedRow(Range::all())=M(Range::all(),i);    
  }
  void CalcAndSaveInverse()
  {
    MInverse=MatrixOps::Inverse(M);
    //    cerr<<"Inverse is "<<MInverse<<endl;
  }
  complex<double>  ColRatio(int colIndex,Array<complex<double> ,1> &col)
  {
  ...
  complex<double>  Ratio_1col_1rowp(vector<int> &colIndices, vector<int> &rowIndices, 
			 ...
			 vector<Array<complex<double> ,1> > &newRows)
  {
    ...
    V_r1c1(Range::all(),1)=newRows[0](Range::all())-M(Range::all(),rowIndices[0]);
    V_r1c1(Range::all(),0)=0.0;
    V_r1c1(colIndices[0],0)=1.0;
    
    U_r1c1(0,Range::all())=newCols[0](Range::all())-M(colIndices[0],Range::all());
    U_r1c1(1,Range::all())=0.0;
    U_r1c1(1,rowIndices[0])=1.0;

    MatrixOps::MatrixMultiply(U_r1c1,MInverse,MInverseU); 
    MatrixOps::MatrixMultiply(MInverseU,V_r1c1,Det_UV); 
    Det_UV(0,0)=Det_UV(0,0)+1.0; 
    Det_UV(1,1)=Det_UV(1,1)+1.0; 
    cerr<<Det_UV<<endl; 
